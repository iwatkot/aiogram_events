...
class TextEventGroup:
    ...
    def buttons(cls) -> MagicFilter:
        if not isinstance(cls.events, list) or not cls.events:  # type: ignore[truthy-function]
...
class CallbackEventGroup:
    ...
    def callbacks(cls) -> MagicFilter:
        if not isinstance(cls.events, list) or not cls.prefix:  # type: ignore[truthy-function]
